...
const UploadPage = () => {
  ...
  const handleAnalyze = async () => {
    try {
      ...
      // If PDF is selected, extract text first
      if (activeMethod === "pdf" && file) {
        contentType = "pdf";
        ...
        try {
          const extractResponse = await fetch(`${FLASK_API_URL}/api/extract-pdf`, {
            method: "POST",
            body: formData,
          });

          if (!extractResponse.ok) {
            const errorData = await extractResponse.json().catch(() => ({ error: "Failed to extract PDF" }));
          ...
          if (!content || content.length < 50) {
            throw new Error("PDF does not contain enough text (minimum 50 characters)");
          }
        } catch (pdfErr) {
          console.error("PDF extraction error:", pdfErr);
          toast.error("PDF extraction failed", {
            description: pdfErr instanceof Error ? pdfErr.message : "Could not read PDF file",
          });
          return;
        }
      }

      // Analyze the content (either text or extracted PDF text)
